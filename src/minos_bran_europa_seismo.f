...
c
c**************************************************************************
      subroutine main(modname,modesfile,eigfile,par_eps,par_wgrav,
     +  par_jcom,par_lmin,par_lmax,par_wmin,par_wmax,par_nmin,par_nmax)
      character*256 filnam,modname,modesfile,eigfile
      real*8 par_eps,par_wgrav,par_jcom,par_lmin,par_lmax
      real*8 par_wmin,par_wmax,par_nmin,par_nmax
  100 format(a256)
      print *,modname(1:lnblnk(modname))
      open(7,file=modname,status='old',form='formatted',iostat=iret)
      print *,modesfile(1:lnblnk(modesfile))
      open(8,file=modesfile,form='formatted',iostat=iret)
      call model(7,8)
      close(7)
      print *,eigfile(1:lnblnk(eigfile))
      ifreq=1
      open(3,file=eigfile,form='unformatted',iostat=iret)
      call wtable(8,3,ifreq,par_eps,par_wgrav,par_jcom,
     +  par_lmin,par_lmax,par_wmin,par_wmax,par_nmin,par_nmax)
      close(8)
      close(3)
c      stop
      end 

      subroutine wtable(iout,ioeig,ifreq,par_eps,par_wgrav,par_jcom,
     +  par_lmin,par_lmax,par_wmin,par_wmax,par_nmin,par_nmax)
...
c MB added one line below
  ...
  100 format('#integration precision =',g12.4,'  root precision =',
     +   g12.4,'  gravity cut off =',g12.4,' rad/s'/,'#mode',
...
c*** RM: COMMENT THIS OUT IF WANT TO NOT WRITE THE MODEL
