...
strcpy(wfd_name,"syndat_out.wfdisc");
...
strcpy(ori_name,"syndat_out.origin");
...
*/
strcpy(sit_name,"syndat_out.site");
...
static PyMethodDef CucssMethods[] = {
    {"system",  convert, METH_VARARGS,
     "Execute a shell command."},
    {NULL, NULL, 0, NULL}        /* Sentinel */
};

static PyObject *CucssError;
PyMODINIT_FUNC
initcucss2sac(void)
{
    PyObject *m;

    m = Py_InitModule("cucss2sac", CucssMethods);
    if (m == NULL)
        return;

    CucssError = PyErr_NewException("cucss.error", NULL, NULL);
    Py_INCREF(CucssError);
    PyModule_AddObject(m, "error", CucssError);
}
