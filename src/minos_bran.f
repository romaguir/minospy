...
c
c**************************************************************************
      subroutine main(modelfile,modesfile,eigfile,par_eps,par_wgrav,
     +  par_jcom,par_lmin,par_lmax,par_wmin,par_wmax,par_nmin,par_nmax)
      character*256 modelfile,modesfile,eigfile
...
c MB added one line below
      print *,modelfile(1:lnblnk(modelfile))
      open(7,file=modelfile,status='old',form='formatted',iostat=iret)
      print *,'output file:'
c      read(5,100) filnam
c MB added one line below
...
c MB added one line below
      ...
      ifreq=1
      if(eigfile(1:4).eq.'none') ifreq=0
